...
const mongodb = async function(): Promise<Db> {
  return (
    await MongoClient.connect("mongodb://localhost:27017/odataserver")
  ).db();
};
...
class ProductsController extends ODataController {
  ...
  *find(@odata.query query: ODataQuery, @odata.stream stream: Writable) {
    ...
    if (typeof mongodbQuery.query.CategoryId == "string")
      mongodbQuery.query.CategoryId = new ObjectID(
        mongodbQuery.query.CategoryId
      );
    let cursor = db.collection("Products").find(mongodbQuery.query, {
...
class CategoriesController extends ODataController {
  ...
  *find(@odata.query query: ODataQuery): any {
    ...
    if (typeof mongodbQuery.query._id == "string")
      mongodbQuery.query._id = new ObjectID(mongodbQuery.query._id);
    let cursor = db.collection("Categories").find(mongodbQuery.query, {
      projection: mongodbQuery.projection,
      skip: mongodbQuery.skip,
      limit: mongodbQuery.limit
    });
...
class NorthwindTypes extends Edm.ContainerBase {
  ...
  @Edm.URLDeserialize(
    (value: string) =>
      new Promise(resolve =>
        setTimeout(_ => resolve(new ObjectID(value)), 1000)
      )
  )
...
class MusicController extends ODataController {
  ...
  find(@odata.filter filter: ODataQuery, @odata.query query: ODataQuery) {
    console.log(
      JSON.stringify(createQuery(query).query, null, 2),
      JSON.stringify(createFilter(filter), null, 2)
    );
    let music = new Music();
...
class ImagesController extends ODataController {
  ...
  *getMembers(@odata.key _: number, @odata.stream stream: Writable) {
  ...
  getData(
    @odata.key _: number,
    @odata.context context: ODataHttpContext,
    @odata.result result: Image
  ) {
    return new ODataStream(fs.createReadStream(result.Filename)).pipe(
      context.response
    );
  }
...
class StreamServer extends ODataServer {
  ...
  async Fetch(
    @Edm.String filename: string,
    @odata.stream stream: Writable,
    @odata.context context: any
  ) {
